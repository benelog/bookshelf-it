...
### p76
> 예를 들면, 초창기의 J2EE 어플리케이션에서는 이따끔 도메인 객체를 모두 "엔티티 빈"으로 구현하곤 했다. 이러한 접근법은 성능과 개발 속도면에서 모두 좋지 않은 결과를 초래했다. 반면 오늘날 우수 실천법은 대부분의 업무 로직을 일반 자바 객체로 구현하면서 구성 단위가 큰(large grain) 객체에 대해서는 J2EE 프레임워크를 사용하는 것이다.
